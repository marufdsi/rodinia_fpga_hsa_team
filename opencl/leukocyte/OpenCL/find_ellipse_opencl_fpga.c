...
float *GICOV_OpenCL(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
	...
	check_error(error, __FILE__, __LINE__);
        ...
        char *krnl_file = "./binary/find_ellipse_kernel.aocx";
        char *krnl_bin;
	const size_t krnl_size = load_file_to_memory(krnl_file, &krnl_bin);
        printf("size%d",krnl_size);
...
void select_device() {
	...
	// Iterate through all available platforms, choosing the first one that has a FPGA
	int i;
	for (i = 0; i < num_platforms; i++) {
	
                printf("This is the %d-th platform\n", i);
		// Create an OpenCL context
		cl_context_properties ctxprop[] = { CL_CONTEXT_PLATFORM, (cl_context_properties) platform_ids[i], 0};
		context = clCreateContextFromType(ctxprop, CL_DEVICE_TYPE_ACCELERATOR, NULL, NULL, &error);

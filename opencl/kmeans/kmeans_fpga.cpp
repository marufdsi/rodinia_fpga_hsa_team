...
{	
	...
	char * kernel_kemans_c  = "kmeans_kernel_c";
	char * kernel_kmeans_swap  = "kmeans_swap";
	char * tempchar = "./kmeans.cl";
        char * krnl_file = "./binary/kmeans_kernels.aocx";
	...
	clFinish(cmd_queue);
        printf("kernel swap is finished\n");
           ...
           float  **new_centers)	
{
	...
	if(err != CL_SUCCESS) { printf("ERROR: Memcopy Out\n"); return -1; }
        printf("read from kernel_c is finished\n");

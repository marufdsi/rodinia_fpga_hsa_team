...
float *OpenClFindNearestNeighbors(
	...
	double totalTime = 0;
        ...
        printf("# Records\tWrite(ms) [size]\tKernel(ms)\tRead(s)  [size]\t\tTotal(ms)\n");
        printf("%d        \t",numRecords);

        // Write Buffer
        eventStart = CLGetTime(writeEvent, START);
        eventEnd = CLGetTime(writeEvent, END);

        printf("%0.6lf [%.2fMB]\t",CLTimeDiff(eventStart, eventEnd),(float)((sizeof(LatLong) * numRecords)/1e6));
        totalTime += CLTimeDiff(eventStart, eventEnd);

        // Kernel
        eventStart = CLGetTime(kernelEvent, START);
        eventEnd = CLGetTime(kernelEvent, END);

        printf("%0.6lf\t",CLTimeDiff(eventStart, eventEnd));
        totalTime += CLTimeDiff(eventStart, eventEnd);

        // Read Buffer
        eventStart = CLGetTime(readEvent, START);
        eventEnd = CLGetTime(readEvent, END);

        printf("%0.6lf [%.2fMB]\t",CLTimeDiff(eventStart, eventEnd),(float)((sizeof(float) * numRecords)/1e6));
        totalTime += CLTimeDiff(eventStart, eventEnd);
...
void printUsage()
{
	...
	printf("\n");
	printf("\n");
	printf("-v[int]      Choose kernel version, must be at the end of argument list)\n");
